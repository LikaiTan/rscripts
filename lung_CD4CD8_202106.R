...
options(future.globals.maxSize= 1024*1024*1024*200)
future::plan(strategy = "multicore", workers = 45)
...
# calculate percent of mitochondrial and ribosam genes
...
saveRDS(CD4CD8, 'CD4CD8_integrated_2021_0718.rds')
...
Feature_rast(CD4CD8,c('patient','integrated_snn_res.1.3','tissue', 'CD4CD8','CCR6', 'RORC', 'FOXP3', 'DPP4', 'IL23R'), sz = 1)
...
Feature_rast(CD4CD8, paste0('integrated_snn_res.', seq(0.6,1.8,0.1)), colorset = 'gg') %T>% figsave('CD4CD8_resolutions.pdf', 600, 400)
Feature_rast(CD4CD8,c('patient','integrated_snn_res.1.3','tissue', 'CD4CD8','CCR6', 'RORC', 'FOXP3', 'DPP4', 'IL23R','BLK', 'CCR2', 'KLRB1', 'RORA'), sz = 0.2,assay = 'RNA')
...
phenotable$Cell_cluster <- factor(paste0('C',c(1:17, 17, 18,19,19,20)), levels = paste0('C', 1:20))
...
clone_exp_umap <- Feature_rast(CD4CD8, 'clonal_expansion',c('patient', 'tissue'),do.label = F, colorset =  c('#DC143C','#9400D3', '#1E90FF', '#FAFAD2'), facetcol = 2) %T>% figsave('clonalexpansion_tissue_patient.pdf',150,270) 

Feature_rast(CD4CD8, 'clonal_expansion',do.label = F, colorset =  c('#DC143C','#9400D3', '#1E90FF', '#FAFAD2'))%T>% figsave('clonalexpansion_UMAP.pdf',120,100) 
...
CD4CD8@meta.data  %<>% mutate(Cell_group = case_when(
  ...
  Cell_cluster %in% c('C7', 'C12') ~'group 4: Tmem',
...
CD4groups <- c('group 2: CD4 helpers', 'group 3: Tcm and Th1','group 4: Tmem','group 6: Th1_17 Trm')
CD8groups <- c('group 6: Th1_17 Trm','group 3: Tcm and Th1','group 4: Tmem','group 5: Trm effector')
...
lungT_Satija <- subset(lung_Satija, cell_type %in% tcells)
dim(lungT_Satija)
...
Feature_rast(lungT_Satija, c( 'disease', 'health_status'), colorset = 'gg')


Feature_rast(lungT_Satija, c('CD4', 'CD8A', 'RORC', 'CCR6', 'FOXP3', 'TOX'))



load('/home/big/tanlikai/Lung/public/GSE162498_NSCLC_CD3_11tumors.rds')
