...
future::plan(strategy = "multicore", workers = 100)
...
figsave <- function (p, filename, w =50, h = 60, 
  ...
  ggsave2( plot = p, filename = filename, device=NULL,
...
L_GDTlung %<>% reduce(.f = merge, project = 'lungGDT')
...
TCRfreqtable <- GDTlung_s@meta.data %>% filter(!is.na(v_gene_TRD))  %>% group_by(Cell_cluster, cdr3_TRD) %>%  summarise(TCRDfreq = n()) %>% 
  arrange(TCRDfreq)

TCRfreqtable %>% filter(Cell_cluster %in% c('c1', 'c2' ,'c5', 'c7', 'c6', 'c3', 'c10')) %>%  
  mutate(Cell_cluster = factor(Cell_cluster, levels = c('c1', 'c2' ,'c5', 'c7', 'c6', 'c3', 'c10'))) %>% 


ggplot(
       aes( x = Cell_cluster, y = TCRDfreq, fill = cdr3_TRD,  stratum= cdr3_TRD, alluvium  = cdr3_TRD))+
...
VD2freqtable <- GDTlung_s@meta.data %>% filter(cdr3_TRD_freq >1 &v_gene_TRD == 'TRDV2' & Cell_cluster == 'c4')  %>%
